...
int signaling_connect(signaling_t *sig, const char *host, int port, const char *room,
    ...
    if (sig->sock == INVALID_SOCKET) {
        ...
        MessageBoxW(NULL, L"创建套接字失败", L"Qmini", MB_OK | MB_ICONERROR);
        return 0;
    }
    ...
    /* non-blocking connect with 5-second timeout */
    {
        ...
        if (ret == SOCKET_ERROR) {
            ...
            if (ret <= 0) {
                ...
                MessageBoxW(NULL, L"连接服务器超时(5s)\n请检查服务器地址和网络", L"Qmini", MB_OK | MB_ICONERROR);
                closesocket(sig->sock);
                sig->sock = INVALID_SOCKET;
                return 0;
            }
    ...
    if (!recv_line(sig->sock, resp, sizeof(resp))) {
        ...
        MessageBoxW(NULL, L"服务器无响应\n请检查服务器是否运行", L"Qmini", MB_OK | MB_ICONERROR);
        closesocket(sig->sock);
        sig->sock = INVALID_SOCKET;
        return 0;
    }
    ...
    if (strncmp(resp, "OK ", 3) != 0) {
        ...
        {
            wchar_t wbuf[256];
            MultiByteToWideChar(CP_ACP, 0, log_buf, -1, wbuf, 256);
            MessageBoxW(NULL, wbuf, L"Qmini 注册失败", MB_OK | MB_ICONERROR);
        }
        closesocket(sig->sock);
        sig->sock = INVALID_SOCKET;
        return 0;
